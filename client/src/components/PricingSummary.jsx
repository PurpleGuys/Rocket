...
export default function PricingSummary() {
    ...
    return (<Card className="sticky top-8 shadow-lg">
      <CardHeader>
      ...
      <CardContent className="space-y-6">
        {/* Service Summary */}
        {bookingData.service && (<div className="space-y-3">
        ...
        {/* Address Summary */}
        {bookingData.address && (<div className="space-y-3 pt-3 border-t border-slate-200">
        ...
        {/* Time Slot Summary */}
        {bookingData.deliveryTimeSlot && (<div className="space-y-3 pt-3 border-t border-slate-200">
            <div className="text-sm">
        ...
        <div className="space-y-3 pt-3 border-t border-slate-200">
          {bookingData.service && (<>
              ...
              {pricing.transportCost > 0 && (<div className="flex justify-between text-sm">
              ...
              {pricing.totalTreatmentCost > 0 && (<>
                  ...
                  {pricing.maxTonnage > 0 && (<div className="text-xs text-slate-500 ml-4">
                      Capacité max: {pricing.maxTonnage}T
                    </div>)}
                  {Object.keys(pricing.treatmentCosts).length > 0 && (<div className="ml-4 space-y-1">
                      {Object.entries(pricing.treatmentCosts).map(function (_a) {
                        var _b;
                        var wasteType = _a[0], cost = _a[1];
                        return (<div key={wasteType} className="text-xs text-slate-500 flex justify-between">
                          <span>{wasteType}:</span>
                          <span>{(_b = cost.totalCost) === null || _b === void 0 ? void 0 : _b.toFixed(2)}€ ({cost.pricePerTon}€/T)</span>
                        </div>);
                    })}
                    </div>)}
                </>)}
        ...
        {/* Action Buttons */}
        {currentStep < 5 && (<div className="space-y-3 pt-3 border-t border-slate-200">
            {currentStep > 1 && (<Button variant="outline" className="w-full" onClick={handlePrevious}>
                <ArrowLeft className="h-4 w-4 mr-2"/>
                Retour
              </Button>)}
            <Button className="w-full bg-primary-600 hover:bg-primary-700" onClick={handleNext} disabled={!canProceed}>
